        ...
        // --- Enemy Logic ---
        function spawnWave() {
             waveNumber++; // Increment wave number when wave actually starts
             updateWaveDisplay(); // Update display immediately
             ...
             // Remove the incorrect floating text call from here
             // createFloatingText(`Wave ${waveNumber} Incoming!`, BASE_X, BASE_Y - 100, 'wave-start');
        ...
        function handleWaveClear() {
            // Grant wave clear bonus for the wave that just ended
            const waveBonus = Math.floor(50 + waveNumber * 5 * DIFFICULTY_SETTINGS[difficulty].resource);
            resources += waveBonus;
            // Explicitly state which wave was cleared
            createFloatingText(`+$${waveBonus} Wave ${waveNumber} Clear!`, BASE_X, BASE_Y - BASE_RADIUS - 30, 'wave-bonus');
            updateResourcesDisplay();

            // waveNumber++; // DO NOT increment here - increment happens in spawnWave

            // Check for win condition (if the wave just cleared was the max wave)
            ...
            // Delay the "Incoming" message slightly for better visual flow
            setTimeout(() => {
                if (!gameOver && gameRunning) { // Check if game is still running before showing
                     createFloatingText(`Wave ${waveNumber + 1} Incoming!`, GAME_WIDTH / 2, GAME_HEIGHT / 2 - 50, 'wave-start'); // Announce the *next* wave number
                }
            }, 500); // 500ms delay
        }
